...
typedef struct {
	...
	cairo_surface_t *pen_pixbuf;
	cairo_surface_t *marker_pixbuf;
          ...
          PpsAnnotationLayerInk *draw)
{
	...
	if (tool == TOOL_PENCIL || tool == TOOL_HIGHLIGHT) {
		...
		// g_slist_free (priv->pending_times);
...
update_cursor (PpsAnnotationLayerInk *draw)
{
	...
	case TOOL_PENCIL:
		if (!priv->pen_pixbuf) {
			...
			priv->pen_pixbuf = pps_document_misc_surface_from_pixbuf (pen_pixbuf);
		}
		icon_surface = priv->pen_pixbuf;
		break;
	case TOOL_ERASER:
	...
	case TOOL_HIGHLIGHT:
	default:
		if (!priv->marker_pixbuf) {
			g_autoptr (GdkPixbuf) marker_pixbuf;
			marker_pixbuf = gdk_pixbuf_new_from_resource ("/org/gnome/papers/icons/scalable/apps/marker-symbolic.svg", NULL);
			priv->marker_pixbuf = pps_document_misc_surface_from_pixbuf (marker_pixbuf);
		}
		icon_surface = priv->marker_pixbuf;
		break;
	}
...
pps_annotation_layer_ink_dispose (GObject *object)
{
	...
	g_clear_pointer (&priv->pen_pixbuf, cairo_surface_destroy);
	g_clear_pointer (&priv->marker_pixbuf, cairo_surface_destroy);
